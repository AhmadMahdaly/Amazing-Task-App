...
class AnbyaaReadingView extends StatefulWidget {
  const AnbyaaReadingView({required this.anbyaa, super.key});
  final Anbyaa anbyaa;

  @override
  State<AnbyaaReadingView> createState() => _AnbyaaReadingViewState();
}

class _AnbyaaReadingViewState extends State<AnbyaaReadingView> {
  double _fontSize = 24;
  ...
  void initState() {
    super.initState();
    _anbyaaCubit = context.read<AnbyaaCubit>();

    _fontSize =
        (CacheHelper.getData(CacheKeys.anbyaaFontSize) as num?)?.toDouble() ??
        24;
    _screenOpacity =
        (CacheHelper.getData(CacheKeys.anbyaaScreenOpacity) as num?)?.toInt() ??
        205;

    final colorValue = CacheHelper.getData(CacheKeys.anbyaaTextColor) as int?;
    if (colorValue != null) {
      _textColor = Color(colorValue);
    }

    final offset =
        (CacheHelper.getData('anbyaa_lesson_${widget.anbyaa.id}_offset')
                as num?)
            ?.toDouble() ??
        0;
  ...
  void dispose() {
    _scrollController
      ...
      ..dispose();
    _anbyaaCubit.saveLastRead(widget.anbyaa);
  ...
  void _saveOffset() {
    CacheHelper.saveData(
      key: 'anbyaa_lesson_${widget.anbyaa.id}_offset',
      value: _scrollController.offset,
    );
  }

  void _goToNextLesson() {
    final nextLesson = _anbyaaCubit.allAnbyaa.firstWhere(
      (l) => l.id == widget.anbyaa.id + 1,
      orElse: () => widget.anbyaa,
    );

    if (nextLesson.id != widget.anbyaa.id) {
          ...
          child: Scaffold(
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Slider(
                                        ...
                                        onChangeEnd: (value) {
                                          CacheHelper.saveData(
                                            key: CacheKeys.anbyaaScreenOpacity,
                                            value: value,
                                          );
                                        },
                                      ),
            ...
            body: AppWallpaper(
              ...
              child: ColoredBox(
                ...
                child: widget.anbyaa.content.isEmpty
                    ...
                    : SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            Text.rich(
                              TextSpan(
                                text: widget.anbyaa.content,
                                // children: _buildHighlightedSpans(
                                //   widget.anbyaa.content,
                                // ),
