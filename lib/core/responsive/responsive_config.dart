...
abstract class SizeConfig {
  ...
  static double getWidth(double width) {
    return SizeConfig.isTablet
        ? screenWidth * ((width * 3) / deviceSize.width)
        : screenWidth * (width / deviceSize.width);
  }

  static double getHeight(double height) {
    return SizeConfig.isTablet
        ? screenHeight * ((height * 3) / deviceSize.height)
        : screenHeight * (height / deviceSize.height);
  }
  ...
  static double getFontSize(double size) {
    ...
    return SizeConfig.isTablet
        ? ((size * 1.3) * scale).clamp((size * 1.3) * 0.85, (size * 1.3) * 1.15)
        : (size * scale).clamp(size * 0.85, size * 1.15);
  }
